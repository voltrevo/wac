# wac-L4 — structs, arrays, enums, and wasm GC

wac-L4 is wac-L3 plus a **type system**, and that is the whole of the difference. wac-L3's compiler
...
- **`u8`**, a byte, legal only as an array element or a struct field.

## `u8[]`, which is the string wac-L4 has

`u8` is a **storage type**, not a value type: wasm has no `i8` value, so `u8` is legal only where a
field or an element goes. Reading one emits `array.get_u` and answers an `i32`; writing one takes an
`i32` and keeps the low byte. A local declared `u8` is refused by the engine, which is where every
other type mistake in this rung is caught.

That gives a byte string that is a **GC array** — built, grown, copied, compared and returned like
any other value, with no allocator and no fixed buffer:
...
String *literals* are still `i32` addresses into linear memory, as in every rung below. Copying one
into a `u8[]` is six lines and is written in wac-L4 rather than built in, because the rung above
needs to *build* names far more often than it needs to quote them.
...
That is also why `.l0` grew a type section with recursive groups. `struct Node { Node[] kids; }`
...
nameless struct per variant holding its fields. That needs no subtyping — which `.l0` does not
...
exhaustiveness check: a value whose tag matches no arm falls out of the match, which is what wac-L4
...
compile time to `call $Point_sum` — no vtable, no receiver in the call, because wac-L4 has no
...
wac-L4 tracks types to *pick an instruction* — which struct to read a field from, whether a local is
...
So a wac-L4 program that assembles and validates cannot have a type error in it. What a checker would
...
in wac-L4 is the next rung's compiler, and every rung below has exactly the same absence — sx traps
...
rung above this one is wac itself, and either wac-L4 gains generics or `packages/wacc/src` stops using
them.
...
One pass, no syntax tree, as in wac-L3 — with two additions that the types force.
...
**The `type` directives are emitted into their own buffer and appended at the end.** `.l0` takes
