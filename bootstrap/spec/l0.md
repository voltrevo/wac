...
A struct field or an array element may also be **packed**: `i8` or `i16`, which are storage types
rather than value types. wasm has no `i8` *value*, so reading one takes `array.get_u`/`array.get_s`
or `struct.get_u`/`struct.get_s` and answers an `i32`. A local declared `i8` is refused by the
engine, which is where every other type mistake in this format is caught, so the assembler does not
duplicate the rule.
...
**References and GC**
    ...
    struct.set $S <field>
    array.new $A             array.new_default $A      array.new_fixed $A <n>
    array.get $A             array.get_s $A            array.get_u $A
    array.set $A             array.len
    ref.func $fn             call_ref $sigtype
    ref.null <$T|none|any|eq|…>
    ref.is_null              ref.eq                    ref.as_non_null
    ref.test <ref|refnull> $T
    ref.cast <ref|refnull> $T

`call_ref` names the **signature**, not the function, which is the whole difference between it and
`call` — the function is on the stack.

**A `ref.func` needs its target declared**, which wasm requires so that an engine knows which
functions can escape. The assembler scans every body for `ref.func` and emits one *declarative*
element segment naming them all, so nothing in the source has to say it twice.
