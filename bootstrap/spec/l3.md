# wac-L3 — the first rung that looks like wac

The point of wac-L3 is the **syntax**. Everything above it is a superset, so this is where the lexer
and the recursive-descent parser get written, once, in wac-L3 — and every rung above inherits them by
...
- **string literals**: `"text"` is the address of a length-prefixed block, exactly as in wac-L3, so
...
in it, which is the only thing wac-L3 is for.
...
to **wac-L4** — which is written in wac-L3 and is the last intermediate before wac itself.
...
**One pass, no syntax tree.** The parser emits `.l0` as it goes, which suits wasm exactly —
...
lets wac-L3 have scopes where wx does not.
