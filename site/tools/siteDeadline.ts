// Run a program that never terminates, with a deadline. Used by `site.test.ts` as a subprocess,
// because a worker stuck in a wasm loop keeps Deno's process alive even after `terminate()` —
// see `runIsolated`. Prints one line and exits; the parent kills whatever is left.
//
// **The compiler arrives as a path.** This compiled with the TypeScript reference until it was
// deleted, and the obvious replacement — call `buildWaccAsset()` here — would climb the whole
// ladder a second time for two four-line programs. The parent has already built the asset, so it
// passes where it put it.
import { createRunner } from "../src/editor/wac-compile.ts";
import { compileWithWacc, type WaccModule } from "../src/editor/wacc-compile.ts";

const ms = Number(Deno.args[0] ?? "800");
const apiPath = Deno.args[1];
if (apiPath === undefined) throw new Error("usage: siteDeadline.ts <deadline-ms> <wacc-api.js>");

const wacc = await import(apiPath) as unknown as WaccModule;

function compiled(src: string, file: string) {
  const r = compileWithWacc(wacc, { [file]: src }, file);
  if (!r.ok) throw new Error(r.diagnostics[0].message);
  return { wasm: r.compiled.wasm, wire: r.wire, sigs: r.sigs };
}

const SPIN = `export i32 spin(i32 n) { i32 i = 0; while (n != 1) { i++; } return i; }`;
const OK = `export string hi() { return "recovered"; }`;

// One runner across both, so the second call also shows that a killed worker is replaced rather
// than reused — it is still spinning and could not answer.
const runner = createRunner();
const started = Date.now();
const stopped = await runner.run(
  { ...compiled(SPIN, "a.wac"), funcName: "spin", argStrings: ["0"] },
  ms,
);
const elapsed = Date.now() - started;
const after = await runner.run(
  { ...compiled(OK, "b.wac"), funcName: "hi", argStrings: [] },
  10_000,
);
runner.dispose();

console.log(JSON.stringify({ elapsed, ...stopped, after: after.output }));
