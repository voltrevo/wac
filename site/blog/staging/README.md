...
| [2026-08-05-an-oracle-you-do-not-control.md](2026-08-05-an-oracle-you-do-not-control.md) | why every test here compares against something written by somebody else; the structural blind spot (the fixture is never the subject, so `seq 1 2 9` printed `1 2` in a suite where a hundred cases use `seq`); reaching an oracle *transitively* when it has no opinion about your code (an in-memory filesystem compared against the host one, whose host side is compared against bash); and the same blind spot one level up — the dead-code check that was wrong five times, and the fix that made it quietly *miss* three findings |
| [2026-08-05-no-closures-no-vtables.md](2026-08-05-no-closures-no-vtables.md) | what a language without closures or virtual dispatch does to a design, and why that was fine — **half expired 2026-08-16, closures landed; carries an editor's note** |
