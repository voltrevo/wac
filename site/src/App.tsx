...
export default function App() {
  ...
  const [active, setActive] = useState(() => {
    ...
    // Prefer a user file (non-example) as initial active
  ...
  const deleteFile = (name: string) => {
    if (name.includes("/examples/")) return; // can't delete examples
  ...
  const renameFile = (oldName: string) => {
    if (oldName.includes("/examples/")) return; // can't rename examples
