...
import { compile, wasmHex, runFunction, placeholderFor, type EditorCompileResult } from "./wac-compile";
...
export default function OutputPanel({ files, fileName }: Props) {
  const [tab, setTab] = useState<"wasm" | "run">("run");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", backgroundColor: "#1e1e2e", borderRadius: 4, border: "1px solid #2e2e3e" }}>
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        {tab === "wasm" ? <WasmView result={result} /> : <RunView result={result} files={files} fileName={fileName} />}
      </div>
    </div>
  );
}
