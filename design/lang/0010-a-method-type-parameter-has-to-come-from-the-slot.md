...
- **Status:** **implemented 2026-08-27 — option C, in use, all six criteria met.**
  `spec/cases/0245`–`0249` are the landed behaviour.

  **The sixth was predicted to be scheduler plumbing and was not.** *What is left to build* item 1
  says a value-returning `then` has to make a second ticket and resolve it when the first resolves. A
  ticket is an id plus three functions of that id, so the chained one keeps the **same host id**, the
  same `settled`, the same `drop` and the same world, and differs only in `resolve`, which gains `f`
  on top — function composition, no scheduler change. What actually stood in the way was compiler
  work the document had marked done: a **lambda** inside a method with its own letters,
  `issues/lang/0277b`. Every spec case here has an own-letter method whose body is ordinary code.
...
1. ~~**`Pending<T>.then` does not exist in the value-returning form**~~ — **done 2026-08-27 as
   `Pending<U> map<U>`**, and the paragraph below is left as written because its prediction was
   wrong in an instructive way: it is composition on the existing ticket, not a derived one. The two
   *measured* facts under it held exactly — the name had to change, and `std/platform.wac` needed
   `wac task gen:core` and a reseed. The original text:

   **`Pending<T>.then` does not exist in the value-returning form**, and this item **understates what
...
| 4 | `p.then<Foo>(…)` | **yes**, as `map` — `std/platform.wac`'s `Pending<U> map<U>(const this, fn[U(T)] f)`, tested in `packages/platform/test/wac/scheduled_test.wac` including two links in a row. Not named `then` because wac has no overloading and the void-consuming `then` has five callers. It needed `issues/lang/0277b` first: a lambda inside an own-letter method |
