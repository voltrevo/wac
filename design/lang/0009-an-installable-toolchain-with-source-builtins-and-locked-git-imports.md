...
binary. `wac self uninstall [--keep-cache]` removes the binary, the cache, the profile line and the
...
same profile line, same `wac self uninstall`. What changes is two sentences of it. *"Builds the native V8
...
8. **`wac:install`, `wac self uninstall`** (D1).
...
| 6. canonical identity (D8) | **done** (2026-08-20). All four spellings the note asks for. `§wac-core-one-key-5jm2qhx` for the built-in trees; `projectspec_test.wac` for `./foo.wac` against `@/src/foo.wac`, with an absolute and a relative entry so normalisation is exercised; `mappedspec_test.wac` for a whole-repository mapping against a `subdir` one, the pair that does *different* suffix arithmetic and must meet at one path; and **two mappings at different commits asserted to be two modules**, which is the negative half and the only one that can fail for D8's own reason — canaried by pointing both at one commit, where the program compiles and answers 4. Asking the question of the first two is what found `issues/lang/0163`. |
...
| 8. `wac:install`, `wac uninstall` (D1) | **done as tasks.** `tools/install.ts` builds the seed fixpoint-checked, installs `bin/wac`, `cache/git/`, `env` and `install.json5` under `$WAC_HOME`, and adds one marked line to the profiles that exist — replaced rather than left when it points at a different home, which is how a reinstall to a new `$WAC_HOME` works. **`wac uninstall` is a subcommand as of 2026-08-19**, spelled `wac self uninstall` since 2026-08-28 — which is what D1 asked for and what a task cannot be: a task is a Deno program under `tools/`, so it needs this checkout, and somebody who installed the command has a `$WAC_HOME` and no checkout. It removes those and the profile line, and nothing else. **The `wac:uninstall` task went on 2026-08-26** and with it the second copy of the layout: it was written down twice, in `tools/install.ts` and in the command, and `packages/wacc/test/wac/uninstall_test.wac` held them to one list with a differential — the same fake install twice, one taken away with each, canaried so two uninstallers that both did nothing could not agree their way to green. Retiring the copy nobody could reach retires the reason for the differential too, on the operator's ruling: a test whose job is to prove the retiree still agrees makes the retiree an oracle. Three cases are left and they ask what the command does — the three files an install writes, the cache with `--keep-cache` and without, and two files it must never touch. `app:native-binary` is renamed (2026-08-18) |

**All eight steps have landed** (step 6 completed 2026-08-20). That sentence read *"nothing has landed"* when the
