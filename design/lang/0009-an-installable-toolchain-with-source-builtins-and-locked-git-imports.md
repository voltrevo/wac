...
| 6. canonical identity (D8) | **three of four spellings** (2026-08-20). `§wac-core-one-key-5jm2qhx` for the built-in trees; `projectspec_test.wac`'s D8 case for `./foo.wac` against `@/src/foo.wac`, with an absolute and a relative entry so normalisation is exercised; and `mappedspec_test.wac`'s whole-repository mapping against a `subdir` one, which is the pair that does *different* suffix arithmetic and has to meet at one path. The fourth — two mappings at different commits asserted to be **two** modules — is the negative half and is open. Asking the question of the first two is what found `issues/lang/0163`. |
