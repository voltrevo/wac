...
| unified binary (V8) | **standardised**: `deno task app:binary` writes one executable with the runtime inside it — 105 MB, 1.02s to compile wacc's own sources, byte-identical to every other path. `packages/wacc/test/wac/binary_test.wac` holds that, opt-in behind `WAC_BINARY=1` because each run writes 105 MB |
