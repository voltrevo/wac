...
| **the unified binary on the primary platform** | **one file compiles wac** — `native/v8/build.rs` embeds `seed/wacc.wasm` and the same binary becomes a command: `wacv8 compile packages/wacc/src/api.wac` writes the compiler's own sources in 67 MB and about 1.2s, byte-identical to every other path, with no Deno and no JavaScript in it. `packages/wacc/test/nativeBinary.test.ts`, opt-in behind `WAC_V8_SEED=1`. It cannot yet rebuild the seed it carries, and the reason has moved: the module it emits is correct, but the `wac.manifest` section that makes a module self-describing is written by `packages/platform/native.ts`, which is TypeScript. The compiler is off the reference; the **bundler** is not |
