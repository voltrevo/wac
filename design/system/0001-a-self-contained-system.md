...
| step | state |
|---|---|
| 1. VFS with two backings | **done.** `packages/fs`, threaded through `packages/sh` as a value the shell holds; `sealed.wac` is a session on `Fs.inMemory()` built with no filesystem grants at all. All 817 scripts of `packages/sh`'s corpus answer identically on **three** backings — memory, image and host — and identically to bash. 0067 is where it started, at 57 on two |
