...
- **Two process models exist.** `spawn`/`spawnSelf` give a real one (a worker, its own instance, its
  own grants); `pushChild`/`popChild` give an in-process one, which is what runs 63 applets in a
...
a shell on `Fs.inMemory()` with all 63 applets and **no capabilities at all**, which is the shape
