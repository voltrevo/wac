...
| 5. line discipline | **the module is done; nothing is wired to it.** `packages/tty` — echo, erase, `^U`, `^W`, `^C`, `^D`, `^V` — with every rule read off the kernel's own discipline through a pty, thirty-one sequences byte for byte. Three of them are not what anyone would write down: erasing `^A` takes two backspaces, `^H` is not erase, and `^W` goes back over letters and digits rather than to the previous space. What is left is the wiring: `sshd` refuses `pty-req` deliberately and this is what it needs before it can accept one, the browser's keydown loop is the other end, and the step's own criterion — `^C` ends a running `yes` — also needs step 3, since interrupting a program takes something that knows what is running |
