...
warned about**, every one of them `100755` in the index and unexecutable on disk, which was
[issues/system/0132](../../issues/system/closed/0132-a-checkout-onto-a-host-mount-cannot-set-the-executable-bit.md)
and nothing else. **That issue is now closed and the warning is gone**: `Cli.setExecutable` sets the bit,
so the same run reports nothing at all. The transcript above is kept as it was recorded rather than
rewritten, because it is the measurement that made the gap concrete. The commit half is step 5. The rows below say what each piece cost and what is still
...
| 4. index and working tree | **done, and the named exception is gone.** `example/gitco.wac` is a checkout as a *program* over `packages/fs`, which is what makes this git in wac rather than wac libraries driven by TypeScript: delete a repository's working tree, run it, and `git status --porcelain` is empty, `ls-files` unchanged, `fsck` clean, on a fixture with nested directories, binary content and a packed object database. **The executable bit was the exception and is now built.** `Cli` gained `setExecutable(path, bool)` and `Stat` gained `isExecutable` — one bit each way rather than a `chmod`, because git's two regular-file modes differ only there, because `mkdir` and `remove` are already `(string, bool)`, and because a page's OPFS has no mode bits at all, so the browser answers `FAULT_UNSUPPORTED` (capability present, backing refuses) where a general mode would be unanswerable. Widening later is additive; narrowing takes something away. Implemented on all four hosts, with the mask arithmetic in three of them and a `gap` entry in the conformance ledger saying only the Deno copy is exercised. **The criterion this row always named is now met without qualification**: delete a repository's working tree, run `gitco`, and `git status --porcelain` is *empty* — the test that used to require the warning and the ` M script.sh` line was written to fail loudly when this landed, and did. [issues/system/0132](../../issues/system/closed/0132-a-checkout-onto-a-host-mount-cannot-set-the-executable-bit.md) is closed. It also made an **unstaged mode change** reportable: a file that gains or loses the bit with every byte unchanged is ` M`, which `worktreeStatus` could not see before and the fixture now moves both ways, since a check written for one direction passes while the other goes unreported. Earlier state of this row: **index done, working tree not.** `src/index.wac` reads and writes version 2 and 3, preserves extension blocks verbatim, refuses version 4 rather than misreading its compressed paths, and verifies the trailing SHA-1. The criterion for the half that is done: replace git's index with ours and `git status --porcelain` is still empty, `ls-files` unchanged, `fsck` clean. The stat-cache risk this document opened with is **settled and not a risk** — a committed index with every stat field zeroed and its checksum repaired still gives an empty status, because git re-reads and re-hashes on a cache miss, and that measurement is kept as a test so the plan cannot quietly stop being true. What is left is checkout: writing a tree to files, which needs the loose-and-packed store the README names. **Two more entry kinds are now named, and one of them was only a prediction until it was measured.** A *symlink* (mode `120000`) cannot be created by `packages/fs` at all, so it lands as an ordinary file holding its target — the fixture had none, so what git says about it was a claim in a source comment rather than a measurement. It now has one, and git says exactly ` T`, a typechange in the **worktree** column, our index having recorded mode 40960 and agreeing with `HEAD`; the test asserts the untrimmed porcelain output, because trimming it would let a wrong index pass as a filesystem limitation. `gitco` counts and reports it, which it previously did not. A *gitlink* (`160000`) is a submodule's commit and so absent from this repository by definition. It **used** to fail the whole checkout — and with `a tree could not be read, or nesting went past 32 deep`, a message about depth for a repository that merely has a submodule. It now does what git does for a submodule it has not initialised: makes the empty directory and records the gitlink in the index, after which `git status` is clean and `git fsck` is happy. The fixture asserts the shape both ways — a `160000` entry in the tree, and that the commit it names is *absent*, since a gitlink pointing at a local commit would take the ordinary read path and prove nothing — and the fix was confirmed by disabling the branch and watching the old failure return. Still missing: reading `.gitmodules` and cloning the submodule. The **symlink** limitation above is now filed as issues/system/0137 and is a decision rather than work — a symlink's contents are a path, every confinement rule here is expressed as a path, and resolution happens below us, so the capability needs a rule about what a target may name before it needs an implementation. **And now read back**: `worktreeStatus` compares a working tree against the index and answers **both** of `git status --porcelain`'s columns — the index against `HEAD`, and the working tree against the index — which `example/gitst.wac` prints in git's own order, so the two are compared with a diff rather than sorted into sets. `MM`, a file staged and then edited again, is the line that proves the columns are computed separately rather than one derived from the other; confirmed by deriving one from the other and watching the test fail. **Untracked files too**, which needed `.gitignore` first — a status listing every build artefact is wrong in a more annoying way than one listing none — so `src/ignore.wac` matches ignore rules and agrees with `git check-ignore` on **882 decisions**, and gathers them the way git does — **all four sources, lowest first**: `core.excludesFile`, `.git/info/exclude`, the root `.gitignore`, then each **nested** one as the walk descends, so a deeper file's rules land later and win. The first of those is supplied by the *program* rather than read by the library, because its value is usually `~/…` and expanding that needs `HOME`, which a `Repo` has no business carrying; `gitst` reads the config, expands it and hands over the bytes. **Its position was measured, not assumed** — with the same path named by two sources at once, `git check-ignore -v` names `.git/info/exclude` as the deciding line over the excludes file, and a `.gitignore` as the deciding line over both — and the fixture is built so all three disagree, since one where they agree passes against a matcher that never read the file at all. Confirmed by reversing the order and by removing the program's `env` grant, each of which turns the test red in its own way. Every rule carries the directory its file sat in, which is what stops a `sub/.gitignore` applying outside `sub`; the test asserts both halves in git's own answer first, since a matcher that ignored nested files entirely would pass a fixture that only checked the negation, generated by crossing a set of names with itself rather than hand-listed, because the 38 cases I chose first were the same person guessing twice and two of them passed while wrong. Two orderings had to be learned from git rather than assumed: porcelain is **grouped**, tracked changes before untracked, not globally sorted; and an entirely untracked directory collapses to one line ending in `/` while a directory holding a tracked file has its untracked contents listed individually — our lines are git's byte for byte on a fixture holding a modified file, one in a subdirectory, a deleted file and an untracked one, minus the untracked one we do not claim to find. Everything in this row until now *wrote*; nothing had read a working tree and compared, so the package could build a repository and not answer a question about one |
| 5. writing a repository git accepts | **done.** `example/gitci.wac` writes blobs, trees, a commit and the branch, and the index in the same pass. **`git fsck --strict` accepts it**, `git log` shows the commit with its parent, and `git status` is empty afterwards — the last of those is more than the criterion asked for and came free once the index was written alongside the ref. A re-commit of the same tree adds no objects, which is what content addressing should give. Root commits — no `parent` header at all — are a separate test. The author now comes from **the config files git actually reads** — `src/config.wac` parses the format and agrees with `git config --list --file` line for line, and `src/configfiles.wac` finds the files: `/etc/gitconfig`, then `${XDG_CONFIG_HOME:-~/.config}/git/config`, then `~/.gitconfig`, then `<gitdir>/config`, with the last of a name winning. That row read "from `.git/config`" for a while and it was true and nearly useless: an identity is a property of the person rather than the repository, so it lives in `~/.gitconfig`, and reading only the local file meant refusing to commit for very nearly everyone who has one configured. The order is `git config --list --show-origin`'s, and two parts of it are not what they look like — **both** global files are read and merge per key rather than the dotfile replacing the XDG one, and `XDG_CONFIG_HOME` being unset does not switch that file off, since git falls back to `~/.config`. The fixture gives all three writable levels a *different* author and removes them one at a time, requiring the next to take over, because one where a single file sets the value passes against a reader that searches in the wrong order or stops at the first hit; `user.email` is left set only at the XDG level throughout, which is what makes the per-key merge load-bearing rather than assumed. Adjudicated by `git log --format='%an <%ae>'` on commits the program wrote, and `git fsck --strict`. Confirmed by two mutations — searching only the local file, and letting the dotfile replace the XDG one — each of which turns it red. `/etc/gitconfig` is read but **not tested**, since a suite cannot write `/etc`, and that is said rather than counted. And the time from the clock. This row used to say there was "no clock and no config reader", and **half of that was wrong**: `Core.nowMillis` had always been there. The real reason was determinism, and git has an answer to it that is not refusing to read a clock: `GIT_AUTHOR_DATE`, which is honoured here and is the lever this repository's own fixtures already pull on real git. An unset `user.name` is a **refusal**, as it is in git, because a commit is signed and inventing a name to put on it is the one thing a tool must not do — and the test asserts nothing was written rather than that a message was printed |
| 6. fetch | **done, against real GitHub, and now incrementally too.** `src/pktline.wac` and `src/fetch.wac`: an advertisement from real `git upload-pack --advertise-refs` parses to the refs `show-ref` lists with capabilities taken off the first line's NUL; a request we build is one `--stateless-rpc` answers; the packfile is extracted from the reply. And **`indexPack` builds an index for a pack that arrived without one** — every name and offset identical to the index git wrote for the same pack, deltas included, and 18,209 objects of this repository's own pack in 2.4 seconds. That needed one addition to `packages/gzip`: `inflateAt`, which reports where a DEFLATE stream ended, because a pack records no object lengths and only the inflater knows. The end is `pos - (bitCount >> 3)`, not `pos` — a bit reader holds bits from bytes it has already pulled. What is left is **the transport**, and it is less missing than this row used to imply. Reading the code rather than assuming: `packages/box`'s `gets` applet already **is** an https client written in wac — TLS over `cli.connect`, then HTTP — so TLS from wac is not the gap and never was. Two things were. **One is now done:** `packages/tls/src/roots.wac` builds a trust store from a PEM bundle, measured against `host/connect.ts`'s `pemBundle` over this system's own 121-certificate `ca-certificates.crt`, byte-identical DER and pair-identical offsets. Before it the whole store could only be built in TypeScript and `gets` took a single pre-converted `.der` on its command line, which is not a clone: cloning from a real host means trusting whichever root signs it, and picking that one out by hand ahead of time is doing the interesting part off-stage. **And so is the other:** the proxy. `packages/http/src/proxy.wac` opens a `CONNECT` tunnel, and `example/tunnel.wac` run against this container's own Squid gets **`open to github.com:443 via gateway:3128`** — a socket to a host outside this container, from wac, for the first time. Its decisions are pure and tested without a socket, the way `packages/http` tests everything else; the loop that owns one is tested by running the program, which is skipped without `HTTP_PROXY` and says so. A refusal keeps the proxy's own line, which is operational rather than decorative: `example.com:443` comes back `HTTP/1.1 403 Forbidden`, so the allowlist is legible instead of arriving as a timeout. **And the wiring is now done for the advertisement half.** `example/gitls.wac` composes all of it — tunnel, trust store, TLS, HTTP, and this package's own pkt-line parser — and against real GitHub it lists **16,209 refs byte-identical to `git ls-remote`** for `ethereum/go-ethereum`. That is the first thing here to leave the machine: a server nobody in this container configured, a chain verified against the system's roots, and a proxy in the way. `test/wac/lsremote_test.wac` keeps it, on a smaller remote, with `git ls-remote` run either side of ours so a repository whose `refs/pull/*` moved mid-test is skipped rather than failed, and with the advertisement's shapes asserted present — a peeled tag, a deep name, a branch, a HEAD — since a remote with one branch would exercise none of them. It also settles a question a pipe cannot ask: the capability list after the first line's NUL, which a parser splitting on spaces would turn into a ref called `HEAD\0multi_ack…`, and which shows up as a diff rather than needing its own case. **And the pack is now done, so this row is.** `example/gitfetch.wac` POSTs the wants, finds the pack in the reply, indexes it, and **reads the commit it asked for back out of the index it built** — against real GitHub: 1,632,748 bytes, 790 objects, `HEAD 321eca5b…689` a commit of 1098 bytes, and that name is the one `git ls-remote HEAD` gives. Content addressing is what makes that an oracle rather than a byte count: a pack parsed wrongly cannot yield an object whose SHA-1 is the name that was wanted, so one lookup exercises TLS, HTTP, pkt-line, the pack format, delta resolution and SHA-1 at once. `wantRequest` gained a depth, and where `deepen` goes is the part worth recording — after the wants and **before the flush that ends the upload-request**, not with the haves that follow it; put it after the flush and the server reads it as a have and silently sends everything. That mistake is what the pipe test asserts against, and it was confirmed by making it: the test fails with `deepen is not between the wants and their flush`. `deepen 1` also keeps the live test at 1.6MB rather than a whole history. Independently confirmed with `curl` before any of it was written: the same byte count, and the reply shape `shallow` · flush · `NAK` · raw `PACK`, which `findPack` already skipped. What is **not** here: `have`-based incremental fetch, which is a thin pack and named in step 2, and side-band, which is a decision above | **And incrementally, which is what finally exercised `completeThin`.** `example/gitpull.wac` sends a `have` and repairs the reply: `gitpull request` writes the want/have request, `gitpull apply` completes the thin pack against the local object store, writes the pack and its index, and records the tip in `FETCH_HEAD`. Adjudicated by three commands rather than one — plain `git index-pack` **refuses** the raw reply with `unresolved delta`, `git fsck --strict` accepts the repository afterwards, and `git rev-parse FETCH_HEAD` resolves to the commit asked for. The refusal is asserted *before* anything of ours is compared, because a server that sent whole objects would make the repair never run and nothing would say so; confirmed by removing the `have` and watching that assertion fire with `git accepted the pack, so it is not thin and this test proves nothing`. **This row previously recorded a thin pack as unarrangeable**, from a measurement against GitHub where `deepen 3` returned 794 whole objects and not one `ref-delta`. That measurement was right and the conclusion drawn from it was not: it is a fact about deepening, not about the protocol. The branch is deliberately not moved — fetching is not merging — and `gitpull` speaks over a pipe rather than HTTP, so an incremental fetch *over the network* is still not written.
| 7. packfiles, write | **done, the small version, as this document instructed.** `writePack` in `src/pack.wac` writes whole objects and no deltas — the plan said to take that and say so, because git's ratios are a search problem (which base, which window, which order) and a push of a few objects is bytes either way. All three of the commands named above adjudicate it, on a pack built from **git's own bytes for git's own commit** rather than objects we invented, which would have agreed with themselves: `git index-pack` accepts it, `git verify-pack -v` lists every object with the type and size git gave it and calls all five `non delta`, and `git unpack-objects` into an empty repository yields every object back under its own name — the re-hash, since a loose object's path *is* its SHA-1 — with `git fsck` clean after. The fixture asserts its own shape first: a commit, a tree, a blob, two trees so nesting is in there, and an object over 2,047 bytes, because four bits of the size live in the first header byte and seven in each one after, so small files would leave the continuation loop unentered. Confirmed by breaking it: a writer that emits no continuation bytes is refused with `pack has bad object at offset 36: inflate returned -5`. **And push, which this row said was not done.** The reason given was that authentication is a non-goal — true of GitHub and not true of pushing at all: `git receive-pack` over a pipe asks for no credentials, so the conversation is arrangeable here and every part of it has an oracle. `src/receive.wac` and `example/gitpush.wac` do it, and `writePack` finally has a caller: seven objects for two commits over a subdirectory, `unpack ok`, `ok refs/heads/main`, the ref moved, `git log` in the target showing both commits in order and `git fsck` clean. The object count is checked against `git rev-list --objects` rather than trusted, because a walk that skipped a parent's tree still yields a pack the server unpacks and a ref update that succeeds — the history would simply arrive incomplete. A **fast-forward** works too, and the old value comes from the advertisement rather than being guessed — which is what makes the update a compare-and-swap. That property is proven rather than asserted: a request built from a *stale* advertisement unpacks perfectly and is still refused, with the ref left where it was, which is also the shape `readReport` exists to distinguish. And the pack now carries **only what the far end lacks**, which is easy to read as needing a negotiation the protocol does not have: `receive-pack` has no `have` exchange, but the advertisement already said where every ref points, and everything reachable from those is present there by definition — so the difference is two local walks. Checked against `git rev-list --objects <new> --not <old>`, which computes the same set: three objects where a full walk sends ten, and the test asserts both the exact count and that something was actually left out, since a fixture where everything is needed would pass either way. Still absent: authentication, so nothing pushes to a host that asks for it, and no `git push` that finds its own remote |
| 8. the clone, as one program | **done, and step 4's exception with it; one thing is a measurement rather than a test.** `example/gitclone.wac` against real GitHub: **790 objects in 1,632,748 bytes, 718 files on `refs/heads/master`, `git fsck` clean, and `git status --porcelain` empty**. It reported exactly one line until `Cli.setExecutable` landed — `scripts/check_fork_comments.py`, `100755` in the index and unexecutable on disk, which was issues/system/0132 and nothing else. With the tolerance removed the test asserts the clone's index holds at least one `100755` entry, because a check that merely stopped allowing a difference would pass on a clone containing no executable at all, and the loop that used to inspect them would simply not run: that is how a test stops testing and keeps passing. It also gets the three things a clone needs that a fetch does not — a `.git` git recognises, the pack named `pack-<its own trailing SHA-1>` as git names it, and **`shallow`**, which is the one easy to forget: a deepened fetch yields a commit whose parent is absent, so without that file `git fsck` calls the repository broken. It has a canary rather than a claim — remove it and fsck must report `broken link … missing commit`, which it does, and restoring it must make fsck clean, which it does. `fetch.wac` gained `symrefOfHead`, because which branch `HEAD` is cannot be inferred: two branches may name one commit, and git guesses `refs/heads/master` where this refuses and says so. **A full-depth clone has now been run**, against `github.com/voltrevo/wac`: **21,065 objects in 10,268,552 bytes, 1,946 files, fourteen seconds**, `git fsck` clean, and `git status --porcelain` reporting 128 lines every one of which is ` M` — the executable bit and nothing else. Checked against the real repository rather than taken on trust: the clone's `HEAD` is `4f938e99`, and that commit has exactly 2007 commits behind it and 21,065 reachable objects there, so what arrived is the whole history rather than a truncation of it. It also exercises the branch the shallow clone cannot — **no `.git/shallow` is written**, because no `deepen` was sent, where the shallow test asserts that file is load-bearing by removing it. Kept as a **measurement rather than a suite test**: ten megabytes and fourteen seconds on a five-core machine three agents share is a cost every push would pay, and `packages/wacc/tools/runOnWacc.ts` sets the precedent for taking a measurement deliberately instead. `depth 1` is what the tests pin. The plan also asked for our object set to be compared against a real clone's; for a shallow clone `git fsck` subsumes it, since a missing object is a broken link and that is what it reports |
