...
| 8. the clone, as one program | **started: the index writer is in.** A clone receives a pack with no index and git will not read one without it, so `src/idx.wac` gained `writeIdx` — and the index it writes for this repository's own pack is **git's own file, byte for byte**. That comparison is worth more than the contents check beside it: names and offsets were already compared, while the CRC-32 per object, the cumulative fanout and both trailing hashes were computed by us and checked against nothing. It found two real defects immediately. The CRCs were **zero** — left so on the reasonable argument that nothing read them, which stopped being true the moment a file git verifies contained them; they are now the CRC of each object's bytes as they sit in the pack, header included, from its offset to the next object's, a range established by measuring git's index rather than by reading a description of the format. And the large-offset table wrote `0x7FFFFFFF` for every entry, because `as~` **clamps** where `as@` truncates — so the low half of any offset past 2 GiB came out as `i32`'s maximum, wrong for exactly the objects that table exists for. No pack here is over 2 GiB, so git will not produce one to compare against; a round-trip through `parseIdx` covers it instead, and that is what caught the cast. What is left for this row: a `.git` skeleton — `HEAD`, `refs/heads/`, `objects/pack/` — then writing the pack and its index into it, then the checkout and index that step 4 already builds |
