...
| 6. fetch | **protocol done, transport not.** `src/pktline.wac` and `src/fetch.wac`: a refs advertisement from real `git upload-pack --advertise-refs` parses to the refs `show-ref` lists with the capability list taken off the first line's NUL; a request we build is one `git upload-pack --stateless-rpc` answers; and the packfile is extracted from the reply, object count matching what the server sent. `side-band-64k` is deliberately not requested, so the reply is one `NAK` and then raw pack bytes rather than an interleaved stream. Two things block a clone, both established by measurement rather than assumed: **a fetched pack carries no index**, and building one needs an inflate that reports bytes consumed, which `packages/gzip` does not export; and **this container has no direct egress** — external DNS fails, everything goes through a Squid proxy, and `Cli.connect` is a raw TCP connect, so a real server needs `CONNECT host:443` spoken to the proxy before TLS. Neither is a decision, so neither is filed; both are named here and in the package README |
