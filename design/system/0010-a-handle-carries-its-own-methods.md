...
- **Decided by:** the operator, 2026-08-31, while settling `vision/SHOWCASE.md`'s stream example
...
`vision/SHOWCASE.md`'s stream example is the one-screen version:
...
`answer(sys, sock)` in `vision/SHOWCASE.md`'s server keeps both parameters, and deliberately: a handler body
