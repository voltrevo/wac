...
| 4. CRYPTO frames and the handshake | **a first flight of our own is accepted; nothing derives from the answer yet.** `src/frame.wac` reads and writes frames, `src/params.wac` encodes the transport parameters, `packages/tls` gained a `clientHello` writer, and `sealClientInitial` puts them in a protected Initial. quinn answers with an ACK and a ServerHello, and the key share is one we hold the private half of. What is left is the key schedule: x25519 against the server's share, then the Handshake secrets — which is where the step's *both sides agree* is decided |
