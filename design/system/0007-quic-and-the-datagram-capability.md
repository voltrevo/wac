...
| 4. CRYPTO frames and the handshake | **reading and sealing are done; no handshake completes.** `src/frame.wac` walks the five frames an Initial may carry and reassembles the CRYPTO stream by offset; `sealClientInitial` builds one. A packet we build is accepted by quinn — a sealed Initial carrying a ClientHello draws a real ServerHello, which `openServerInitial` opens under the id the client invented. What is left is authoring a ClientHello we hold the key share for, which is what the step's own definition needs |
