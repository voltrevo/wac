...
| 4. CRYPTO frames and the handshake | **reading is done; nothing is sent yet.** `src/frame.wac` walks the five frames an Initial may carry and reassembles the CRYPTO stream by offset. quinn's first flight decrypts to a ClientHello `packages/tls` parses — 32-byte random, TLS 1.3, a key share it knows, `TLS_AES_128_GCM_SHA256`. What remains is the half that speaks: writing an Initial, protecting it, and driving the TLS state machine to Handshake keys |
