...
| 4. CRYPTO frames and the handshake | **done: both sides agree on the Handshake secrets.** We author a first flight — `packages/tls`'s `clientHello`, `src/params.wac`'s transport parameters, an x25519 share we hold — seal it, and quinn answers with a coalesced Initial and Handshake. `src/keys.wac` runs the server's share through TLS's schedule and QUIC's three labels, the Handshake packet opens to EncryptedExtensions, and the server's Finished verifies — which says the transcripts match and not merely the keys. Nothing answers yet: no Finished is sent, so no connection completes and no 1-RTT keys exist |
