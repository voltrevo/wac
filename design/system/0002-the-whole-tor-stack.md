...
### The relay half of step 6 was never written down

Step 6 is done when the suite stands up a network with no C in it, publishes an onion service on it,
and fetches a page from that service. Building `hsserviced` made it obvious that the second half of
that sentence has no implementation behind it.

`relayd` implements **none** of the onion-service relay roles. Only the client and service sides exist
anywhere in the package; `relay.wac` defines the command numbers and nothing consumes them. So a
service in this repo can establish an introduction point only against a C tor, and two relays are
missing from every path an onion service uses:

| role | what it must do | state |
|---|---|---|
| introduction point | accept ESTABLISH_INTRO, file the circuit under its auth key, forward INTRODUCE1 down it as INTRODUCE2, ACK the sender | `parseEstablishIntro` only |
| rendezvous point | accept ESTABLISH_RENDEZVOUS, remember cookie → circuit, match RENDEZVOUS1, forward as RENDEZVOUS2, then splice the two circuits | nothing |

Neither is named in this document's step list, which is how it stayed invisible: every entry in the
step-6 row was about a *service* doing something, and a service doing something requires a relay
willing to be done to.

**And both roles need a value the relay does not have.** ESTABLISH_INTRO is MACed with KH, the nonce
at the tail of the ntor expansion. The client half of that was missing too and is now fixed; the
...
### The matrix caught its own author

Step 7 is a document, which is why it kept being last: nothing in it is new work, and a table that
restates what five other sections already say is easy to file under *later*. It earned its place on
the first pass.

Step 6's line in this document is **"done when a C tor client reaches a service we host."** The run
recorded above is *our* client reaching it. That meets the deliverable's condition at the top of this
file — a network with no C in it, a service published on it, a page fetched through three hops — and
it is a different claim, and I had marked the step done against the wrong one.

The error is not that the run proves nothing. Every cell along that path was pinned separately against
cells C tor wrote, so the composition was the only new assertion, and `joined: the service is hop 4`
means an hs-ntor AUTH check passed between two ends that derived key material from opposite sides of
one expansion. That is worth having. It is *not* a C tor on the other side of a socket, and the two
were one word apart in a table.

**A matrix is how a claim gets compared against its own definition.** Prose lets "done" travel without
the condition attached — which is the same failure as the one the security note found in the root
README, a capability quoted without its limit, one level up. The table has a column for the direction
and a row for the component, so a claim has nowhere to hide between them.
...
0107](../../issues/system/open/0107-a-c-tor-fetching-from-our-onion-service-times-out-intermittently.md).
