...
| verified resolution, composed — `packages/ens/src/verified.wac` | **done for the registry** — a name in, an address out, through `namehash`, Solidity's mapping layout, the state trie and the account's storage trie, with the storage root taken from the account proof rather than from the caller. Anvil's `eth_getProof` for the fixture, `cast` for the two derivations. The resolver's own storage is still not implemented, so what is verifiable is ownership and the resolver's address |
