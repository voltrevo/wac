...
// **Its own module, and the reason is 44 KiB.** A constant array becomes one immutable global built
// by `array.new_fixed`, with every element an `i32.const` in the initialiser — about five bytes each
// — and every constant in a *module* is emitted once that module is linked, used or not. So a
// program that asked `tables.wac` only whether a code point is printable was paying for the 8,790
// entries of case mapping as well: `box`'s `wc` measured 503 KiB against 460 for `sha256sum` beside
// it. Splitting the file is the whole fix; nothing about the tables changed.
const printableOut = `// Generated by tools/gentables.ts. Do not edit.
//
// ${printable.length} ranges of printable code points, sorted and disjoint: \`PRINT_KEYS[i]\` is the first of a
// range and \`PRINT_VALUES[i]\` the last. Printable means assigned and not a control, a surrogate or a
// line/paragraph separator — \`iswprint\` in a UTF-8 locale, and ${printableCount} code points answer yes.
//
...
await Deno.writeTextFile("packages/unicode/src/printable.wac", printableOut);
