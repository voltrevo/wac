...
// `ByteQueue` now lives in `./queue.ts`, as a pure state machine plus a driver, so that every
// interleaving of push, read, end and cap-driven parking can be enumerated rather than sampled by luck.
// Imported here because this file is what wires a child's three streams together.
