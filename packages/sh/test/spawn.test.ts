...
Deno.test("a file that is not a worker bundle is a failed command, not a dead shell — 0021", async () => {
  ...
  // infer it from a `SyntaxError`: the file is neither of the two things a program can be. It is
  // answered from the bytes, before a worker starts, which is also what stopped a bundle that *parses*
  // and never speaks the protocol from wedging the shell — 0033.
  assertEquals(r.err.includes("not a wac program"), true, r.err);
});
