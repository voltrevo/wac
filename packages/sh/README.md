...
`test/corpus.ts` holds **883** scripts. `test/differential.test.ts` runs the **589** of them that
...
this package's: **every script naming an external program is filtered out**, so of the 589 cases
...
and the argument for changing it was the oracle rather than taste: 883 corpus scripts are compared
