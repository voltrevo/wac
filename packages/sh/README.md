...
`test/corpus.ts` holds **867** scripts. `test/differential.test.ts` runs the **573** of them that
...
this package's: **every script naming an external program is filtered out**, so of the 573 cases
...
and the argument for changing it was the oracle rather than taste: 867 corpus scripts are compared
