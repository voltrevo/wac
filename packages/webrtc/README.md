...
crosses to a browser and back, split across chunks and put together again. But nothing keeps a timer
of its own, so recovery rides on the peer's, and there is no congestion control. Both state machines are structs in wac now — `Peer` for the DTLS
