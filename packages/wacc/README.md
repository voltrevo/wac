...
| `specMulti.test.ts` | the spec's 56 programs that take more than one file | **the contract** — 6 of 15 illegal refused, 40 of 41 legal silent, the rest named |
...
holds them now: 6 of the 15 illegal ones are refused and the nine misses are *named*, which is two
rules rather than nine bugs — a name that is declared without `export` is not the file's to give away
(four), and writing a type's name requires importing it (five). The single false alarm is the same gap
seen from the other side: a declaration table keyed by bare name cannot tell one file's `Point` from
another's, so `§wac-samename-struct-4jhq7wn` loses the second one.
