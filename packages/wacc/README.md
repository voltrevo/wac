...
| `specMulti.test.ts` | the spec's 56 programs that take more than one file | **the contract** — all 15 illegal refused, 40 of 41 legal silent |
...
The single false alarm left is a different rule: `§wac-samename-struct-4jhq7wn`, where two modules each
declare a `Dup` and one is imported under an alias. Names are declared under the name the *declaring*
file gave them, so the alias is never a type and the second module's fields are lost to the first.
That one wants per-file identity rather than another rule.
