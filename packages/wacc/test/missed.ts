// The programs `mutateCheck.test.ts` counts as missed, printed instead of tallied.
//
//     deno run -A --unstable-net packages/wacc/test/missed.ts "undefined variable"
//
// The argument is a substring of the reference's message, so it picks one row out of that test's
// table; with none it shows the first ten of everything. Not a test — a tally names a rule and this
// names the program, which is the difference between knowing recall is 96% and being able to fix it.
// Kept beside the sweep it reads from, because the two share the mutation list and the pair of
// compilers, and a copy of that in a scratch file goes stale the day the mutations change.
//
// What follows is `mutateCheck.test.ts`'s own harness, imported rather than duplicated where that was
// possible: the mutation list is local to it, so this file redeclares it and any change there needs
// the same change here — which the header of that file now says.
//
...
for (let i = 0; i < cells.length; i++) {
  ...
  try { theirs = reference(mutated); } catch { continue; }
  if (theirs === null || theirs.length === 0) continue;
  if (ours(mutated).length > 0) continue;
  missed++;
  if (want !== "" && !theirs[0].message.includes(want)) continue;
  if (shown >= 10) continue;
  shown++;
  console.log(`--- the reference says ${theirs[0].line}:${theirs[0].col} ${theirs[0].message}`);
  console.log(mutated.split("\n").map((l) => "    " + l).join("\n"));
}
console.log(`(${shown} shown of ${missed} missed in all)`);
