...
// Resolved through the `wac/` alias, which is `./compiler/` since the merge — so this is
// `spec/tour.wac` at the repository root, and no longer a sibling checkout.
...
/**
 * Every `.wac` file in the repo, including `spec/tour.wac`.
 *
...
export async function corpus(): Promise<Corpus> {
  ...
  try {
    files.push(["spec/tour.wac", await Deno.readTextFile(TOUR)]);
  } catch (e) {
    skipped.push({ name: "spec/tour.wac", reason: (e as Error).message });
  }
