...
35 packages, 99,248 lines of wac, 1740 tests,
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 19,962 | 107 | `bytes` |
...
| [`git`](packages/git/) | git in wac: the object database, and enough of a client to fetch from a real server. | 3,750 | 42 | `bytes` `codec` `crypto` `fmt` `fs` `gzip` `http` `platform` `std` `tls` |
