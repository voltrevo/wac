...
35 packages, 100,707 lines of wac, 1753 tests,
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 20,123 | 110 | `bytes` |
...
| [`git`](packages/git/) | git in wac: the object database, and enough of a client to fetch from a real server. | 4,991 | 52 | `bytes` `codec` `crypto` `fmt` `fs` `gzip` `http` `platform` `std` `tls` |
