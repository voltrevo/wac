...
35 packages, 98,878 lines of wac, 1732 tests,
...
| [`git`](packages/git/) | git in wac: the object database, and enough of a client to fetch from a real server. | 3,596 | 39 | `bytes` `codec` `crypto` `fmt` `fs` `gzip` `http` `platform` `std` `tls` |
