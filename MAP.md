...
39 packages, 125,168 lines of wac, 2464 tests,
...
| [`crypto`](packages/crypto/) | The hashes, AEADs, curves and one KEM the rest of this repo is built on — written in wac, calling nothing. | 5,890 | 249 | `bignum` `codec` `fmt` `platform` |
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 29,708 | 214 | `bytes` `codec` `crypto` `fmt` `fs` `platform` `std` |
