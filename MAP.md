...
38 packages, 130,990 lines of wac, 2567 tests,
...
|---|---|---|---|---|
| [`bytes`](packages/bytes/) | `Buf` — a growable byte buffer. | 339 | 39 | — |
...
| [`regex`](packages/regex/) | A backtracking regular expression engine, with JavaScript's semantics — and the two POSIX dialects `grep` reads, translated into them rather than… | 1,530 | 23 | `bytes` |
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 34,596 | 271 | `bytes` `codec` `crypto` `fmt` `fs` `wacpkg` |
