...
39 packages, 124,849 lines of wac, 2351 tests,
...
| [`platform`](packages/platform/) | A capability world for wac applications, so a program can be written **entirely in wac** — no TypeScript of its own — and still read files, tell… | 6,090 | 216 | `bytes` `fmt` |
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 29,637 | 214 | `bytes` `codec` `crypto` `fmt` `fs` `platform` `std` |
