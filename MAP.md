...
39 packages, 125,060 lines of wac, 2462 tests,
...
| [`crypto`](packages/crypto/) | The hashes, AEADs, curves and one KEM the rest of this repo is built on — written in wac, calling nothing. | 5,890 | 252 | `bignum` `codec` `fmt` `platform` |
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 29,657 | 214 | `bytes` `codec` `crypto` `fmt` `fs` `platform` `std` |
...
| [`wacpkg`](packages/wacpkg/) | Where a wac project says its dependencies come from: `wac.json5`, read and checked. | 1,082 | 29 | `bytes` `fmt` `json` `platform` `wacc` |
