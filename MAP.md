...
35 packages, 100,349 lines of wac, 1749 tests,
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 20,116 | 110 | `bytes` |
...
| [`sh`](packages/sh/) | A shell, in wac, whose definition of *correct* is GNU bash: a corpus of scripts runs through both and the two must agree on standard output… | 7,129 | 30 | `bytes` `codec` `fmt` `fs` `platform` `std` |
...
| [`git`](packages/git/) | git in wac: the object database, and enough of a client to fetch from a real server. | 4,675 | 48 | `bytes` `codec` `crypto` `fmt` `fs` `gzip` `http` `platform` `std` `tls` |
