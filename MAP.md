...
35 packages, 95,570 lines of wac, 1691 tests,
54 command-line programs and 5 browser pages.
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 18,817 | 97 | `bytes` |
...
| [`git`](packages/git/) | git's object database, in wac. | 1,732 | 21 | `bytes` `codec` `crypto` `fmt` `fs` `gzip` `platform` `std` |
