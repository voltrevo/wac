...
37 packages, 114,953 lines of wac, 2028 tests,
...
| [`platform`](packages/platform/) | A capability world for wac applications, so a program can be written **entirely in wac** — no TypeScript of its own — and still read files, tell… | 5,232 | 171 | `bytes` `fmt` |
...
| [`quic`](packages/quic/) | QUIC version 1 — RFC 9000 and RFC 9001 — in wac. | 3,251 | 86 | `crypto` `platform` `tls` |
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 25,376 | 173 | `bytes` `codec` `crypto` `fmt` `fs` `platform` `std` |
| [`webrtc`](packages/webrtc/) | WebRTC in wac — the data channel half, following `design/system/0008`. | 3,563 | 72 | `crypto` `fmt` `gzip` `platform` `tls` |
