...
38 packages, 120,375 lines of wac, 2210 tests,
...
| [`platform`](packages/platform/) | A capability world for wac applications, so a program can be written **entirely in wac** — no TypeScript of its own — and still read files, tell… | 5,731 | 194 | `bytes` `fmt` `std` |
...
| [`tor`](packages/tor/) | Tor in wac, both ends: a client and SOCKS5 proxy, a relay, a directory authority, an onion-service client, and a test network with no C tor in it. | 16,094 | 309 | `bytes` `codec` `crypto` `datetime` `fmt` `http` `platform` `std` `tls` `wactest` |
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 28,816 | 206 | `bytes` `codec` `crypto` `fmt` `fs` `platform` `std` |
