...
33 packages, 72,579 lines of wac, 1447 tests,
...
| [`sh`](packages/sh/) | A shell, in wac. | 7,096 | 39 | `bytes` `codec` `fmt` `fs` `platform` `regex` `std` |
...
| [`tor`](packages/tor/) | Tor in wac, both ends: a client and SOCKS5 proxy, a relay, a directory authority, an onion-service client, and a test network with no C tor in it. | 15,988 | 294 | `bytes` `codec` `crypto` `datetime` `fmt` `http` `platform` `std` `tls` `wactest` |
