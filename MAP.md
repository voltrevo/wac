...
39 packages, 123,952 lines of wac, 2322 tests,
...
| [`zstd`](packages/zstd/) | Zstandard (RFC 8878) in wac. | 3,033 | 56 | `bytes` |
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 29,405 | 211 | `bytes` `codec` `crypto` `fmt` `fs` `platform` `std` |
