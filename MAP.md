...
34 packages, 76,656 lines of wac, 1527 tests,
48 command-line programs and 4 browser pages.
...
| [`platform`](packages/platform/) | A capability world for wac applications, so a program can be written **entirely in wac** — no TypeScript of its own — and still read files, tell… | 3,426 | 116 | `bytes` |
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 6,902 | 56 | `bytes` |
...
| [`fs`](packages/fs/) | A filesystem that belongs to the system rather than to the host. | 2,065 | 34 | `bytes` `fmt` `gzip` `platform` `std` |
...
| [`sh`](packages/sh/) | A shell, in wac. | 5,709 | 25 | `bytes` `codec` `fmt` `fs` `platform` `std` |
...
| [`box`](packages/box/) | 62 applets in one program, chosen by the first argument — 61 tools and `help`, which prints the list. | 6,973 | 75 | `bytes` `codec` `crypto` `datetime` `fmt` `fs` `gzip` `http` `json` `platform` `regex` `server` `sh` `std` `tls` `unicode` `url` `zstd` |
