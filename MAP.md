...
33 packages, 71,719 lines of wac, 1436 tests,
...
| [`regex`](packages/regex/) | A backtracking regular expression engine, with JavaScript's semantics. | 1,201 | 18 | `bytes` |
...
| [`fs`](packages/fs/) | A filesystem that belongs to the system rather than to the host. | 1,459 | 29 | `bytes` `fmt` `gzip` `platform` `std` |
...
| [`sh`](packages/sh/) | A shell, in wac. | 7,036 | 37 | `bytes` `fmt` `fs` `platform` `regex` `std` |
...
| [`box`](packages/box/) | 61 applets in one program, chosen by the first argument — 60 tools and `help`, which prints the list. | 5,292 | 35 | `bytes` `codec` `crypto` `datetime` `fmt` `fs` `gzip` `http` `json` `platform` `regex` `server` `sh` `std` `tls` `unicode` `url` `zstd` |
...
| [`tor`](packages/tor/) | Tor in wac, both ends: a client and SOCKS5 proxy, a relay, a directory authority, an onion-service client, and a test network with no C tor in it. | 16,024 | 294 | `bytes` `codec` `crypto` `datetime` `fmt` `http` `platform` `std` `tls` `wactest` |
...
| `packages/sh/src/imaged.wac` | A shell whose filesystem is a file: the session ends, the filesystem does not. |
