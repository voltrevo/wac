...
38 packages, 131,201 lines of wac, 2597 tests,
...
|---|---|---|---|---|
| [`bytes`](packages/bytes/) | `Buf` — a growable byte buffer. | 361 | 40 | — |
...
| [`tty`](packages/tty/) | What a terminal does to your keystrokes before a program sees them: echo, erase, kill, word erase, `^C`, `^D`. | 496 | 8 | `bytes` |
...
| [`json`](packages/json/) | JSON (RFC 8259) parsing and serialization, written in wac — and JSON5, as a second entry point. | 1,321 | 57 | `bytes` `fmt` |
| [`platform`](packages/platform/) | A capability world for wac applications, so a program can be written **entirely in wac** — no TypeScript of its own — and still read files, tell… | 4,305 | 222 | `bytes` `fmt` |
...
| [`crypto`](packages/crypto/) | The hashes, AEADs, curves and one KEM the rest of this repo is built on — written in wac, calling nothing. | 7,029 | 291 | `bignum` `codec` `fmt` `wactest` |
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 34,768 | 290 | `bytes` `codec` `crypto` `fmt` `fs` `wacpkg` |
...
| [`wactest`](packages/wactest/) | Assertions for tests written in wac. | 2,103 | 34 | `bytes` `crypto` `fmt` `json` |
