...
39 packages, 124,569 lines of wac, 2341 tests,
...
| [`zstd`](packages/zstd/) | Zstandard (RFC 8878) in wac. | 3,033 | 63 | `bytes` |
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 29,505 | 211 | `bytes` `codec` `crypto` `fmt` `fs` `platform` `std` |
| [`wactest`](packages/wactest/) | Assertions for tests written in wac. | 643 | 17 | `bytes` `crypto` `fmt` `json` `platform` |
...
| [`wacpkg`](packages/wacpkg/) | Where a wac project says its dependencies come from: `wac.json5`, read and checked. | 852 | 24 | `bytes` `json` `wacc` |
