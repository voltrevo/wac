...
35 packages, 95,129 lines of wac, 1686 tests,
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 18,723 | 94 | `bytes` |
...
| [`git`](packages/git/) | git's object database, in wac. | 1,385 | 19 | `bytes` `codec` `crypto` `fmt` `gzip` `std` |
