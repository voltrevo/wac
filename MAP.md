...
33 packages, 72,189 lines of wac, 1440 tests,
...
| [`sh`](packages/sh/) | A shell, in wac. | 7,087 | 38 | `bytes` `fmt` `fs` `platform` `regex` `std` |
...
| [`box`](packages/box/) | 61 applets in one program, chosen by the first argument — 60 tools and `help`, which prints the list. | 5,711 | 38 | `bytes` `codec` `crypto` `datetime` `fmt` `fs` `gzip` `http` `json` `platform` `regex` `server` `sh` `std` `tls` `unicode` `url` `zstd` |
