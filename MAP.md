...
33 packages, 73,406 lines of wac, 1465 tests,
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 4,137 | 22 | `bytes` |
...
| [`fs`](packages/fs/) | A filesystem that belongs to the system rather than to the host. | 1,635 | 34 | `bytes` `fmt` `gzip` `platform` `std` |
...
| [`sh`](packages/sh/) | A shell, in wac. | 6,984 | 39 | `bytes` `codec` `fmt` `fs` `platform` `regex` `std` |
...
| [`ssh`](packages/ssh/) | An SSH-2 client and server, in wac, **and `ssh` and `sshd` programs built from them. | 3,629 | 50 | `bytes` `codec` `crypto` `fmt` `fs` `platform` `sh` `std` |
