...
35 packages, 103,825 lines of wac, 1804 tests,
...
| [`platform`](packages/platform/) | A capability world for wac applications, so a program can be written **entirely in wac** — no TypeScript of its own — and still read files, tell… | 4,903 | 161 | `bytes` `fmt` |
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 21,630 | 126 | `bytes` `codec` `crypto` `fmt` `fs` `platform` `std` |
