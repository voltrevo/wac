...
35 packages, 98,811 lines of wac, 1731 tests,
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 19,755 | 103 | `bytes` |
...
| [`git`](packages/git/) | git in wac: the object database, and enough of a client to fetch from a real server. | 3,529 | 38 | `bytes` `codec` `crypto` `fmt` `fs` `gzip` `http` `platform` `std` `tls` |
