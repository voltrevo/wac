...
35 packages, 94,537 lines of wac, 1676 tests,
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 18,702 | 92 | `bytes` |
...
| [`git`](packages/git/) | git's object database, in wac. | 814 | 11 | `bytes` `codec` `crypto` `fmt` `gzip` `std` |
