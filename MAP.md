...
38 packages, 134,750 lines of wac, 2783 tests,
...
| [`platform`](packages/platform/) | A capability world for wac applications, so a program can be written **entirely in wac** — no TypeScript of its own — and still read files, tell… | 4,411 | 245 | `bytes` `fmt` `json` `wacc` |
...
| [`wacc`](packages/wacc/) | Porting the wac compiler to wac, so it can eventually compile itself. | 37,481 | 340 | `bytes` `codec` `crypto` `fmt` `fs` `json` `wacpkg` |
