...
39 packages, 123,894 lines of wac, 2314 tests,
...
| [`sh`](packages/sh/) | A shell, in wac, whose definition of *correct* is GNU bash: a corpus of scripts runs through both and the two must agree on standard output… | 7,387 | 30 | `bytes` `codec` `fmt` `fs` `platform` `std` |
