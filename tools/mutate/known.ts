...
export const KNOWN_SURVIVORS: KnownSurvivor[] = [
  ...
  // other check. Here there are two of them, and between them they cover the whole index range.
  {
    ...
    why:
      ...
      "and `items[i]!` traps on the null. `packages/json/test/wac/bounds_test.wac` drives both routes — " +
      "the one-item array's index 1 is deliberately inside the allocation — and cannot distinguish " +
      "them, because " +
  ...
  // ── wacc's token-kind table ─────────────────────────────────────────────────
  {
    ...
    why:
      ...
      "`packages/wacc/test/lex.test.ts`, which derives the names from `compiler/wacLex.ts` at run " +
