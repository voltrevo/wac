...
// unchanged, and the first `wac task seed` after this moves the seed's bytes once and then settles.
...
const BANNER = (from: string[]) =>
  `Generated by \`wac task gen:core\` from ${from.join(", ")}. Do not edit.`;

async function waccFile(): Promise<string> {
  // **Every input, not only the root.** This named `WACC_ROOT` alone, so the one line stating where
  ...
  return `// ${BANNER([...WACC_ROOT, ...SIBLINGS, ...STD])}
//
...
// **One output, since 2026-08-28.** This also wrote compiler/wacCore.ts, the reference's copy of
// the same declarations, and generating both from one source was how they were kept from drifting.
// There is one compiler now, so there is one copy and nothing to drift from.
  ...
  if (stale.length > 0) {
    console.error(
      `${stale.length} generated file(s) do not match \`core/\`:\n  ${stale.join("\n  ")}\n\n` +
        "Run `wac task gen:core`. Editing the embeddings by hand is what this replaces — the two " +
        "differ by design and the difference is which files each one is built from.",
    );
    Deno.exit(1);
  }
  console.log(`core/: the embedding is in step`);
} else if (wanted.every(([p]) => p)) {
  console.log("core/: the embedding written");
}
