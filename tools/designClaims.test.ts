...
async function designDocs(): Promise<{ path: string; text: string }[]> {
  ...
  for await (const entry of Deno.readDir("design")) {
    if (!entry.isFile || !entry.name.endsWith(".md")) continue;
    out.push({ path: `design/${entry.name}`, text: await Deno.readTextFile(`design/${entry.name}`) });
  }
...
Deno.test("the checks above have something to check", () => {
  ...
  const text = Deno.readTextFileSync("design/0001-a-self-contained-system.md");
  assertEquals(
