...
// (since deleted), and it imported neither tools/size.ts nor tools/validate.ts, and `deno run` has
...
async function typescriptFiles(): Promise<string[]> {
  ...
  const walk = async (dir: string): Promise<void> => {
    for await (const e of Deno.readDir(dir)) {
      ...
      if (e.isDirectory) {
        // `site/` is the website — React, .tsx and vite-resolved specifiers — which Deno cannot
        // type-check and which has its own `tsc -b`. It came inside this walk when the language
        // and package repositories merged; `deno.json`'s `exclude` says the same thing for the
        // walks Deno does itself.
        if (e.name === "node_modules" || e.name === ".git" || e.name === ".cache" ||
            e.name === "site" || e.name === "target") continue;
        await walk(path);
