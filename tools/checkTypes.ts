...
async function typescriptFiles(): Promise<string[]> {
  ...
  const walk = async (dir: string): Promise<void> => {
    for await (const e of Deno.readDir(dir)) {
      ...
      if (e.isDirectory) {
        // `site/` is the website: React, `.tsx`, npm imports and extensionless specifiers that vite
        // resolves. Deno cannot type-check it and should not try — it has its own `tsc -b`, run by
        // `site/package.json` and by the Pages workflow. It came inside this walk when the two
        // repositories merged.
