// Every relative link in every markdown file points at something that is there.
//
...
Deno.test("every relative markdown link points at a file that exists", async () => {
  const files = (await tracked()).filter((f) => f.endsWith(".md"));
  const present = new Set(await tracked());
  const broken: string[] = [];
  for (const f of files) {
    const dir = dirname(f);
    for (const t of targets(await Deno.readTextFile(f))) {
      const target = resolve(dir, t);
      // A directory link — `](compiler/)` — is fine if anything tracked lives under it.
      const isDir = t.endsWith("/") || [...present].some((p) => p.startsWith(target + "/"));
      if (present.has(target) || (isDir && [...present].some((p) => p.startsWith(target + "/")))) {
        continue;
      }
      broken.push(`${f}: ${t}`);
    }
...
Deno.test("the check has something to check", () => {
  ...
  const found = targets("see [the design](../design/system/0001-a.md) and [E.A(1)](x) and [n](3)");
  assertEquals(found, ["../design/system/0001-a.md"]);
...
Deno.test("the corpus is the whole repository, not a sample", async () => {
  ...
  const files = (await tracked()).filter((f) => f.endsWith(".md"));
  assertEquals(files.length > 100, true, `only ${files.length} markdown files found`);
  assertEquals(files.includes("README.md"), true, "the root README is not in the corpus");
});
