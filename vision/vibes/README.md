...
Everything added to `vision/` after `vision/packages` was created, moved here because none of it was
vetted.

`vision/` is meant to hold content the operator and whoever is writing have agreed on. Until the
packages exercise began on 2026-09-04 it was six files and 1,549 lines that had been through that.
Over the next forty-one hours the exercise added **43,577 lines in 252 files across 477 commits**,
none of it agreed, all of it into the same directory. `vision/packages` was meant to be the single
exception to the vetting rule; instead the exception became most of the directory.

So the vetted six are back at [`../`](..), byte for byte as they were, and everything since is in
here.

## What this means for a reader

**Nothing in this directory is agreed.** It is the record of an exercise — forty package rewrites,
what they needed and what they could not say — and its findings are real and worth reading. They are
not decisions, and nothing here should be cited as one.

**Three files share a name with a vetted one**, and in each case the copy here is the vetted file
*written over*, not an addition beside it:

| here | at `../` | what happened |
|---|---|---|
| `QUESTIONS.md` | 125 lines, 8 questions | grew to 11,072 lines and 215 entries; the original eight were rewritten in place |
| `TECHNICAL.md` | 876 lines, 34 entries | 37 entries, with edits scattered through the original 34 |
| `DECISIONS.md` | 54 lines, 3 rules | **no new rules** — 41 lines of annotation inside two of the three |

Where they disagree, `../` is the authority. This copy is kept whole rather than reduced to a diff
because the new material is interleaved with the old and does not survive being excerpted.

The rest — `GRAMMAR.md`, `GRAMMAR.ebnf`, `core/`, `std/`, `bench/`, `packages/` and `wac.json5` —
did not exist before and has no counterpart above.

The text below is that README as it stood, unchanged apart from four paths that now point up a
level.
