...
> **Not vetted.** This is `../QUESTIONS.md` as the packages exercise left it: 215 entries against
> the vetted eight, which were rewritten in place. See [README.md](README.md).
...
## A constant map is not a thing, and the three generated files in the tree are the workaround

*2026-09-05.* Three files in `packages/` carry the header *"Generated by … Do not edit."*, and all
three are **parallel key and value arrays with a search over them**:

| file | arrays | size |
|---|---:|---|
| `packages/unicode/src/tables.wac` | 6 | `LOWER` 1,459 pairs, `UPPER` 1,450, `FOLD` 1,481 |
| `packages/unicode/src/printable.wac` | 2 | 733 pairs |
| `packages/raster/src/font16.wac` | 5 | 3,240 glyphs; `GLYPH_ROWS0` is 8,192 elements, the largest literal in the repository |

**Five logical constant maps, eleven arrays.** Nothing here is a `Map<i32, i32>`, and the reason is
not that `Map` is missing — it is that a `Map` is **built**. A table of 1,459 entries built by
inserting 1,459 times at module start is work a constant does not need, and a generated file is
exactly where the contents are known before the program runs.

### The pairing is a parameter, which is worse than a comment

`tables.wac` writes the binary search **once**, and that is the sharp version rather than a
mitigation:
...
`lookup(i32[] keys, i32[] values, i32 cp)` takes two arrays that must be the same length in the same
order. Three such pairs exist. **`lookup(LOWER_KEYS, UPPER_VALUES, cp)` compiles and answers plausible
nonsense**, and no bound is violated because the arrays are within thirty of each other in length.

This file has counted *paired values with nothing holding the pair* many times, at 4,390 pairs this is
the largest, and it is the **first where the pair travels through a signature** rather than sitting in
a struct. A struct is a pair somebody could still build wrongly once; a parameter list is a pair every
caller rebuilds.

### And the generator is the caller nobody consulted

`../README.md`'s test — *name a caller and say what it would do differently* — is **vacuous** here.
The caller is `tools/gentables.ts`, and it would do whatever it is told.

> **A generated file is the one place a representation can be changed without asking anyone**, and it
> is where three of the largest parallel-array cases in the repository live. Every other instance of
> this shape has an author with an opinion; these have a program.
...
`const Entry[] LOWER = Entry[](Entry(65, 97), …)` is what a table wants and nothing here establishes
whether it is legal, when the objects are built, or what that costs at module instantiation. The
array literal is settled; the element type is not, and `spec/` does not say.

**And `printable.wac` is a range table, not a map**, which the one type does not cover. Its pairs are
*(start of range, value for that range)* and the query is *the last range beginning at or before
`cp`*. Two generated files, two shapes, and a single `Entry { from; to; }` names one of them wrongly.
