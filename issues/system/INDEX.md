...
|---|---|---|---|
| [0220](open/0220-twenty-six-copies-of-struct-rng-in-five-variants.md) | 33 `.wac` files declare their own `struct Rng`; 25 are one byte-identical xorshift32 split 13/12 by two different `upto`, which disagree on **37% of draws**, so unifying moves a corpus — and twelve claimed to match a host-side generator their package no longer has (corrected 2026-08-20) | task | wrong answer |
