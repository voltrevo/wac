...
|---|---|---|---|
| [0235a](open/0235a-a-per-file-cpu-figure-includes-its-chunks-shared-build.md) | a directory's test files share one build and the chunk's build is charged to whichever file the runner reports first — `describewac_test` is 4.1s alone and 14.6s beside one other file, so a per-file figure is not that file's cost and two runs compare only if chunk membership held. I filed a compiler regression off these numbers and withdrew it; what survives is `names_test` alone exceeding 900s against a declared 84s | bug | wrong answer |
