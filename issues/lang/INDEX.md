...
| [0266c](open/0266c-a-second-default-is-refused-and-a-repeated-case-value-is-not.md) | both compilers refuse two `default:` arms and name the rule; neither refuses two identical `case` values, which is unreachable code for the same reason and is accepted with the first arm winning. `errDuplicateCase` already refuses the same fault one construct along — a variant cased twice in a `match` — so `switch` is the odd one of three neighbours. They agree, so no differential sees it, and `spec/spec/control.md` does not settle it | decision | unreachable code accepted silently |
