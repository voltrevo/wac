...
|---|---|---|---|
| [0245a](open/0245a-a-string-literal-joined-to-a-string-literal-has-no-type.md) | the slot half is fixed — a string or bool literal contributes its type now, so `i32 n = "a" + "b";` is refused and the chain `t.a + t.b + t.c` still reports once. What is left is the *operand* rule, which asks `alt`/`art` where `nl`/`nr` fill a literal in: pointing it at those makes `true + false` report and makes `n * "a"` report twice, so the question is which path should own that one | bug | wrong answer |
