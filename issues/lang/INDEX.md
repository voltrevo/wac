...
|---|---|---|---|
| [0268c](open/0268c-a-checked-cast-between-signed-and-unsigned-does-not-check.md) | `i32 x = -1; x as! u32` returns **0** where the spec says it traps — `casts.md` line 223 is that exact program with the comment *"traps if x is negative"* — and the reference traps. `emitCast` skips its round-trip guard when `valType(from) == valType(to)`, which is true for exactly `i32`/`u32` and `i64`/`u64`: the bail reads as "nothing to convert" and means "nothing to check". Eight cells of the rung-4 sweep, invisible because the sweep skipped a cell whole when the *reference* trapped | bug | wrong answer where a trap is specified |
