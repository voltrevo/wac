...
|---|---|---|---|
| [0260c](open/0260c-a-generic-at-an-enum-is-declined-in-one-position-and-mis-emitted-in-another.md) | `ready<T>` at an enum from `core` gives the instantiation a second type index, so the function returns a reference type its own signature does not name and the engine refuses the module. The same construct in an *assignment* is declined correctly — so the checker has the rule and a `return` against a declared return type does not reach it | bug | invalid module, no diagnostic |
