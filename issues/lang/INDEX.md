...
|---|---|---|---|
| [0246a](open/0246a-a-ternarys-branch-agreement-rule-has-four-narrow-literal-guards.md) | `b ? 1 + 1 : "a"` is accepted while `b ? 1 : "a"` and `b ? 1 : "a" + "b"` are refused — the branch-agreement rule has four literal guards, all `litKindOf`, and for a compound literal on the left every one of them declines to speak. They interlock: moving one changes which is silent, not the answer | bug | wrong answer |
