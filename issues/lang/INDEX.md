...
|---|---|---|---|
| [0174a](open/0174a-waccs-checker-loses-a-method-lookup-when-the-receiver-is-shadowed.md) | a same-scope redeclaration poisons the name to unknown, so every rule goes silent about it; retyping instead is wrong because the name table is position-blind — diagnosed, corpus cleared | bug | a missing compile error |
...
| [0161](open/0161-an-aliased-import-of-an-already-imported-type-is-a-different-type-in-wacc.md) | a name imported both plainly and aliased loses its plain spelling entirely, so every rule about it goes silent — including arity on a *function*, which was live in `packages/ssh` | bug | a missing compile error |
