...
|---|---|---|---|
| [0267c](open/0267c-a-project-import-in-a-non-entry-file-is-still-silent.md) | `issues/lang/0175a` fixed a `@/` import in the **entry** — every mistake behind one was invisible to `wac check`. The same import one file further in is still silent: `b.wac` importing `@/src/point.wac` and asking for a field that does not exist draws nothing, where the relative spelling draws *no such field* at the right column, and the build then declines with no position. `checkFilesWithIn` resolves the **entry's** specifiers through the `Res` and no one else's, so the type never arrives and unknown is silent by design. `@/` exists for files that are not the entry, so the uncovered half is the half the feature is for | bug | no error, then an emitter decline with no position |
