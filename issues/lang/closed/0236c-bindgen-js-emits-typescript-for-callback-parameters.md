...
## The third criterion, taken literally

Merged from agent-a, who reached the same report independently. The import in `gluerun.ts` is what
caught the defect — a `.js` that does not parse fails at `await import` — but until now nothing *called*
a callback, so the slot table, the dispatcher's forwarding and `$fnref`'s de-duplication were only ever
type-checked by eye. `gluerun.ts` now passes a JavaScript function in and checks the answer:

    if (mod.apply((n) => n * 3, 7) !== 21) …

Parsing and forwarding are separate failures, and the generated dispatcher is the only export shape
whose glue is a function definition rather than a call. Nothing was wrong with the forwarding; the point
is that the test could not have said so.
