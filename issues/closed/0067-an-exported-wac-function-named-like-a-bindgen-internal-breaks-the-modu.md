...
`export i32 _exports()` or `_wasm()` collides with the generated module's own bindings:
`Individual declarations in merged declaration '_exports' must be all exported or all local`.
**Verified.**
...
The module's own bindings are `$wasm`, `$instance`, `$exports`, and the wasm-level helpers are `$bind$…`/`$trap$message`. `export i32 _exports()` and `export i32 _wasm()` are legal wac exports again.
...
The GitHub thread (#6) is still open; close it there too.
